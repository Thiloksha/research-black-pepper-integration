...
export default function SignUpScreen({ navigation }) {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.screen}>
      <LinearGradient colors={['#f0f7f2', '#e2f0e8', '#d4ead9']} style={styles.gradient}>
        ...
        >
          {isLarge ? (
            <View style={styles.desktopWrap}>
              ...
              <Animated.View style={[styles.formCard, { width: cardWidth, opacity: fadeAnim, transform: [{ translateY: slideAnim }] }, shadowCard]}>
                {/* No state props — form manages its own state */}
                <SignUpForm navigation={navigation} isSmall={isSmall} />
              </Animated.View>
            </View>
          ) : (
            <Animated.View style={[styles.mobileWrap, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
  ...
  return (
    <View>
      <View style={styles.formHeader}>
        <Text style={[styles.formTitle, isSmall && { fontSize: 22 }]}>Create account</Text>
        <Text style={styles.formSubtitle}>Sign up to start using PepperSense AI</Text>
      </View>
      ...
      <View style={styles.fieldWrap}>
        ...
        <View style={[styles.inputWrap, nameFocused && styles.inputWrapFocused]}>
          <View style={styles.inputIconWrap}>
            <Ionicons name="person-outline" size={18} color={nameFocused ? '#2d7a4f' : '#9ab0a2'} />
          </View>
          <TextInput
            style={styles.input}
            value={fullName}
      ...
      <View style={styles.fieldWrap}>
        ...
        <View style={[styles.inputWrap, passFocused && styles.inputWrapFocused]}>
          ...
          <TouchableOpacity style={styles.eyeBtn} onPress={() => setShowPassword(!showPassword)} activeOpacity={0.7}>
            <Ionicons name={showPassword ? 'eye-outline' : 'eye-off-outline'} size={18} color="#9ab0a2" />
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.fieldWrap}>
        ...
        <View style={[styles.inputWrap, confirmFocused && styles.inputWrapFocused]}>
          ...
          <TouchableOpacity style={styles.eyeBtn} onPress={() => setShowConfirmPassword(!showConfirmPassword)} activeOpacity={0.7}>
            <Ionicons name={showConfirmPassword ? 'eye-outline' : 'eye-off-outline'} size={18} color="#9ab0a2" />
          </TouchableOpacity>
        </View>
      </View>

      {/* Create Account Button */}
      <TouchableOpacity style={[styles.signInBtn, shadowBtn]} onPress={handleSignUp} activeOpacity={0.85}>
        <LinearGradient colors={['#38a169', '#2d7a4f']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.signInBtnGradient}>
...
const styles = StyleSheet.create({
  ...
  scrollContentLarge: { paddingVertical: 60, paddingHorizontal: 40 },
  ...
  dividerVertical: { width: 1, height: 560, backgroundColor: '#d0e8d8', marginRight: 48, opacity: 0.7 },
  ...
  logoCircle: {
    ...
    borderWidth: 1.5, borderColor: '#d0e8d8',
    ...Platform.select({
  ...
  brandName: { fontSize: 22, fontWeight: '900', color: '#0f2618', marginBottom: 10, letterSpacing: -0.3 },
  brandTagline: { fontSize: 15, color: '#4a6857', lineHeight: 23, marginBottom: 32 },
  featureList: { gap: 14, marginBottom: 32 },
  featureItem: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  featureIconWrap: { width: 32, height: 32, borderRadius: 10, backgroundColor: '#e8f5ee', justifyContent: 'center', alignItems: 'center' },
  featureText: { fontSize: 14, color: '#2a4a38', fontWeight: '600' },
  statsBadge: {
    backgroundColor: '#ffffff', borderRadius: 16, paddingVertical: 12, paddingHorizontal: 20,
    borderWidth: 1, borderColor: '#d0e8d8', alignItems: 'center',
    ...Platform.select({
  ...
  statsBadgeVal: { fontSize: 24, fontWeight: '900', color: '#2d7a4f' },
  statsBadgeLabel: { fontSize: 12, color: '#6b8c78', fontWeight: '600', marginTop: 2 },
  formCard: { backgroundColor: '#ffffff', borderRadius: 28, padding: 32, borderWidth: 1, borderColor: '#e0ede5' },
